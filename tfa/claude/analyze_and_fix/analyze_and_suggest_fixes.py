...
try:
    ...
    print("Error: ollama module not found. Please install it: pip install ollama")
    sys.exit(1)
...
def check_model_exists(model_name: str) -> bool:
    ...
    try:
        ...
        model_names_with_tag = [m['name'] for m in models]

        # Check with and without :latest suffix
        return (model_name in model_names or
                model_name in model_names_with_tag or
                f"{model_name}:latest" in model_names_with_tag)
    except Exception:
        return False
...
def print_available_models():
    ...
    if not models:
        ...
        print("📥 Pull a model first: ollama pull llama3.1")
        return
...
class TestFailureOrchestrator:
    ...
    def __init__(self, model: str = "llama3.1", timeout: int = 120, max_workers: int = 3):
        """
        ...
        """
        self.analyzer = FailureAnalyzer(model=model, timeout=timeout)
        self.suggestor = FixSuggestor(model=model, timeout=timeout)
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive Test Failure Analysis and Fix Suggestions using Ollama",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # List available models
  python analyze_and_suggest_fixes.py --list-models

  # Full analysis with fix suggestions (parallel)
  python analyze_and_suggest_fixes.py test_failures.json

  # Use a specific Ollama model
  python analyze_and_suggest_fixes.py input.json --model codellama
  python analyze_and_suggest_fixes.py input.json -m mistral
  python analyze_and_suggest_fixes.py input.json -m qwen2.5-coder:7b
  ...
  python analyze_and_suggest_fixes.py --sample
    ...
    parser.add_argument("--model", "-m", default="llama3.1",
                       help="Ollama model to use (default: llama3.1). Can be any model name like 'mistral', 'codellama', 'qwen2.5-coder:7b', etc.")
    parser.add_argument("--list-models", action="store_true", help="List all available Ollama models and exit")
    ...
    parser.add_argument("--timeout", type=int, default=120, help="Ollama timeout in seconds (default: 120)")
    ...
    try:
        ...
        if args.list_models:
            ...
            print("\n💡 Usage: python analyze_and_suggest_fixes.py input.json --model <model-name>")
            return 0
        ...
        if not args.input_file:
            parser.error("input_file is required (or use --sample or --list-models)")

        print(f"🚀 Comprehensive Test Failure Analysis using Ollama")
        print(f"📁 Input: {args.input_file}")
        print(f"🤖 Model: {args.model}")

        # Check if model exists
        if not check_model_exists(args.model):
            print(f"\n⚠️  Warning: Model '{args.model}' not found locally.")
