...
def process_json_file(input_file: str, output_file: str = None, framework: str = None, max_failures: int = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        output_file: Optional output file for results (defaults to analysis_results.json)
    ...
    try:
        ...
        for i, test_case in enumerate(failed_tests, 1):
            ...
            result['framework'] = test_framework
        ...
        output_filename = output_file or "analysis_results.json"
        save_results(summary, output_filename)
...
def save_results(summary: Dict[str, Any], output_file: str):
    ...
    output_data = {
        ...
        "analysis_summary": {
            "total_tests": summary["analysis_metadata"]["total_tests"],
            "failed_tests": summary["analysis_metadata"]["failed_tests"], 
            "analyzed_tests": summary["analysis_metadata"]["analyzed_tests"],
            "successful_analyses": summary["analysis_metadata"]["successful_analyses"],
            "failed_analyses": summary["analysis_metadata"]["failed_analyses"],
            "average_confidence": round(summary["analysis_metadata"]["average_confidence"], 2)
        },
        "test_analyses": []
    }
    ...
    for result in summary["results"]:
        if result["success"]:
            analysis = {
                ...
                "original_test": {
                    "failure_message": result["original_test"].get("failure_message", ""),
                    "status": result["original_test"].get("status", "failed")
                },
            ...
            analysis = {
                ...
                "error": result["error"],
                "original_test": {
                    "failure_message": result.get("original_test", {}).get("failure_message", ""),
                    "status": result.get("original_test", {}).get("status", "failed")
                },
                "analyzed_at": datetime.now().isoformat()
            }
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Analyze all failures in JSON file (saves to analysis_results.json)
  python3 analyze_json_failures.py test_results.json
  
  # Save results to specific output file
  python3 analyze_json_failures.py input.json --output my_results.json
...
Supported JSON formats:
  ...
  [{"name": "test1", "failure_message": "..."}]
    ...
    parser.add_argument("--output", "-o", help="Output file for analysis results (default: analysis_results.json)")
    ...
    parser.add_argument("--json-only", action="store_true", help="Output only raw JSON results to stdout")
    ...
    try:
        ...
        if args.json_only:
            ...
            if summary["success"]:
                print(f"\n✅ Analysis Complete!")
                print(f"📊 Summary:")
                print(f"  Total tests: {summary['analysis_metadata']['total_tests']}")
                print(f"  Failed tests: {summary['analysis_metadata']['failed_tests']}")
                print(f"  Successful analyses: {summary['analysis_metadata']['successful_analyses']}")
                print(f"  Average confidence: {summary['analysis_metadata']['average_confidence']:.2f}")
